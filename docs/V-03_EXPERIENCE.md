...
Repo born at extraction: `vynkor-core/vynkor-manager`, binary `vynm`, lib
...
| `.github/workflows/ci.yml` | Mirrors kernel gates + the no-kernel-dep assertion (`cargo tree -i vynkor` must fail = empty inverted tree). |
...
- **Env namespace:** `VYN_STATE_DIR`/`VYN_PLUGIN_DIR` →
  ...
  (`~/.local/share/vyn/`, `~/.local/lib/vyn/plugins`) — vynm manages
  the same tree the kernel spawns from.
- **Error type:** hand-rolled `VynkorError` → thiserror `VynmError`. New repo,
...
| `cargo tree -i vynkor` empty | ✓ (CI asserts it permanently) |
...
2. **V-05**: install pipeline port (+ D2/D3 deletions during the port) —
   ...
   `vynkor_wire::manifest::validate_manifest` with `default_resolver`.
